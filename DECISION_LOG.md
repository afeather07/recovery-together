...
Connected the official Supabase MCP (OAuth-based, no API key ever entered in chat). Used Aaron's already-created "Recovery Together" project (ca-central-1) rather than creating a new one. Applied the full schema (including the 2026-08-03 security/cost fixes) as a single migration. Verified via list_tables (8 tables, all RLS-enabled) and get_advisors (security). One real finding from the advisor run: `enforce_author_rate_limit()`, a trigger-only function, was publicly callable via PostgREST's auto-exposed RPC endpoint -- fixed by revoking EXECUTE from anon/authenticated/public; trigger firing is unaffected since triggers are invoked by the executor, not by a role-based function call. Remaining advisor output (two warnings, one info) is expected and documented in FOUNDER_ACTION_ITEMS.md. One manual step remains outside connector reach: Aaron must enable Anonymous Sign-Ins in the Supabase dashboard himself.
